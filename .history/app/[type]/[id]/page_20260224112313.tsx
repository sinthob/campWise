...
const TYPE_CONFIG: Record<DetailType, TypeConfig> = {
  campground: {
    ...
    insightKeys: {
      ...
      tips: ["Tips", "Recommendations", "What to bring", "What to bring / Tips"],
    },
    quickFactsExclude: [
      "AI Summary",
      "AI Insight",
  ...
  gear: {
    ...
    aiSummaryKeys: ["AI Gear Tip", "AI Summary", "AI Insight", "Summary", "Description"],
    rawReviewKeys: ["Raw Review", "Raw Reviews", "Reviews", "Review", "Notes"],
    insightKeys: {
...
function getAnyAttachmentImageUrl(fields: Record<string, unknown>, preferredKeys: string[]) {
...
function parseAiInsightJson(raw: string | undefined | null): {
  ...
  try {
    ...
    if (strengths.length === 0 && weaknesses.length === 0 && bestFor.length === 0 && tips.length === 0) {
      return null;
    }
...
export default async function DynamicDetailPage(props: {
  ...
  const aiSummaryLooksJson =
    !!aiSummaryTrimmed && (aiSummaryTrimmed.startsWith("{") || aiSummaryTrimmed.startsWith("["));

  const parsedInsight = parseAiInsightJson(fallbackAiSummary);
  const strengthsList =
    parsedInsight?.strengths?.length ? parsedInsight.strengths : toStringListFromUnknown(strengths);
  const weaknessesList =
    parsedInsight?.weaknesses?.length ? parsedInsight.weaknesses : toStringListFromUnknown(weaknesses);
  const bestForList =
    parsedInsight?.bestFor?.length ? parsedInsight.bestFor : toStringListFromUnknown(bestFor);
  const tipsList = parsedInsight?.tips?.length
    ? parsedInsight.tips
    : toStringListFromUnknown(tips ?? (aiSummaryLooksJson ? undefined : fallbackAiSummary));

  const hasAnyInsight =
    strengthsList.length > 0 || weaknessesList.length > 0 || bestForList.length > 0 || tipsList.length > 0;
        ...
        {quickFacts.length > 0 ? (
          <section className="rounded-2xl border border-moss/30 bg-forest p-6 text-sand">
            ...
            <dl className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
              {quickFacts.map((f) => (
                <div key={f.key} className="rounded-xl bg-forest/60 p-4 ring-1 ring-moss/30">
                  <dt className="text-xs font-semibold text-sand/70">{f.key}</dt>
                  <dd className="mt-1 text-sm font-medium text-sand">{f.value}</dd>
                </div>
              ))}
            </dl>
          </section>
