...
export default function HeroGallery(props: {
  ...
  return (
    <section
      ...
      className="overflow-hidden rounded-3xl border border-moss/30 bg-forest shadow-sm"
    >
      {images.length > 0 ? (
          ...
          {images.length > 1 ? (
            <ul className="grid grid-cols-4 gap-3" aria-label="More photos">
              {images.slice(1, 5).map((img) => (
                <li
                  key={img.url}
                  className="overflow-hidden rounded-xl bg-moss ring-1 ring-moss/40"
                >
        ...
        <div className="flex h-[320px] w-full items-center justify-center bg-moss text-sm text-sand/70 sm:h-[420px]">
          No image available
        </div>
      )}
